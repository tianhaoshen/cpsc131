...
{//get the private member in Book class
	_isbn = isbn;
	_title = title;
	_author = author;
	_price = price;
...
//overload the stream in operator to read the file use char common to separate the txt.
std::istream& operator >> (std::istream& stream, Book& book)
{
	...
	stream >> std::quoted(PickBook._isbn) >> delim1
		>> std::quoted(PickBook._title) >> delim1
		>> std::quoted(PickBook._author) >> delim1
		>> PickBook._price;
...
std::ostream& operator << (std::ostream& stream, const Book& book)
{
	...
	stream << std::quoted(book.isbn()) << delim2
		<< std::quoted(book.title()) << delim2
		<< std::quoted(book.author()) << delim2
		<< book.price();
