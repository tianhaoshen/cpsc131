...
int main(int argc, char* argv[])
{
	...
	Book book1("any", "Like the Animals", "9780895656926");
	Book book2("any", "131 Answer Key", "54782169785");
	Book book3("any", "Les Mis", "0140444300");
	Book book4("Asher", "Eat pray love", "9780399576775");
	Book book5("any", "Hunger Games", "9780545310581");
	myCart.push(book1);
	myCart.push(book2);
	myCart.push(book3);
	myCart.push(book4);
	...
	std::cout << storeDataBase.size() << std::endl;
	while (!checkoutCounter.empty()) {
		auto item = storeDataBase.find(checkoutCounter.front().isbn());
		if (item != nullptr) {
			std::cout << "ISBN: " << std::quoted(item->isbn()) << ", Title: " << std::quoted(item->title())
				<< ", Author: " << std::quoted(item->author())
				<< ", Price: $" << item->price() << std::endl;
			amountDue += item->price();
			// std::cout << amountDue << std::endl;
		}
		checkoutCounter.pop();
	}
	/////////////////////// END-TO-DO (7) ////////////////////////////
